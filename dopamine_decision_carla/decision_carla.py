...
class ReplayBuffer(object):
	...
	def __init__(self, observation_shape,  observation_dtype,  action_shape,  action_dtype, reward_shape, reward_dtype, terminal_dtype, replay_capacity, stack_size):
		...
		self.zero_transition = self._make_zero_transition()  # function's () should be remember
	...
	def _make_zero_transition(self):
		...
		return zero_transition   # seems like it can be a constant
	...
	def _add(self, transition):
		...
		for arg_name in ("observation", "action", "reward", "terminal"):
	...
	def save(self, checkpoint_dir, iteration_number):
		...
		checkpointable_elements = ('observation', 'action', 'reward', 'terminal')
	...
	def _is_full(self):
		return self.add_count % self.replay_capacity == self.replay_capacity
...
def main():
	...
	dqn = DQN()
	buffer = ReplayBuffer(observation_shape=(256,256,3),  # but obs.shape = (256,256,3)
	...
	for iteration in range(1,100):  # try to reload world per iteration
		...
		for episode in range(100):
			...
			transition = buffer.make_transition(obs, a, r, int(terminal))
			# buffer.add(transition)
			print("buffer add transition successfully even if this sounds like baby steps")
			reward += r
			obs=obs_
		...
		if buffer._is_full():
			buffer.save("/home/chaselwan/公共的/gym-carla-master/buffer_dataset",iteration_number)
			iteration_number += 1
			buffer_has_been_saved = True
	if buffer_has_been_saved == False:
		buffer.save("/home/chaselwan/公共的/gym-carla-master/buffer_dataset",iteration_number)

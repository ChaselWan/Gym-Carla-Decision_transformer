...
#obs_size are replaced to 80 and 60
#are not replaced
    ...
    # Create an actor_list to store and destroy actor conveniently
    self.actor_list = []
    self.walker_list = []
  ...
  def reset(self):
    ...
    while True:  # ?while true?
      if ego_spawn_times > self.max_ego_spawn_times:
        print("****************ego_spawn_times > smax_ego_spawn_times, so reset**************")
        self.reset()
      ...
      if self._try_spawn_ego_vehicle_at(transform):
        ...
        ego_spawn_times += 1
        print("********************ego_spawm_times:{}".format(ego_spawn_times))
        time.sleep(0.1)
    ...
    self.collision_hist = []
    print("self.collision_sensor:",self.collision_sensor)
    ...
    self.time_step=0
    self.reset_step+=1
    print("**********************updating times:{}*********************".format(self.reset_step))
  ...
  def step(self, action):
    ...
    return (self._get_obs()['birdeye'], self._get_reward(), self._terminal(), copy.deepcopy(info))
  ...
  def _try_spawn_ego_vehicle_at(self, transform):
    ...
    for idx, poly in self.vehicle_polygons[-1].items():
      ...
      if dis > 8:
        ...
        overlap = True
        print('######################ego position has been overlapped')
        break
  ...
  def _get_obs(self):
    ...
    birdeye = birdeye[0:self.display_size, :, :]  # three axis
  ...
  def _clear_actors(self):
    """Clear all actors and walkers"""
    for i,_ in enumerate(self.actor_list):
      if self.actor_list[i] is not None and self.actor_list[i].is_alive:
        # print('Destroying actor {}'.format(self.actor_list[i].id))
        self.actor_list[i].destroy()
      self.actor_list[i] = None
    self.actor_list = []

    for j,_ in enumerate(self.walker_list):# learn and copy, and find how sensor.destroy
      if self.walker_list[j] is not None and self.walker_list[j].is_alive:
        # print("Destroying walker {}".format(self.walker_list[j].id))
        self.walker_list[j].stop()  
        self.walker_list[j].destroy()
      self.walker_list[j] = None
    self.walker_list = [] 
"""
  def train(self):
    self.training = True
  ...
  def _train_get_obs(self, state): # (256,256,3)
    # 把state从array变为torch并除以255
    return torch.tensor(state, dtype=torch.float32, device=self.device).div_(255)

  def train_reset_buffer(self, action):
    for _ in range(self.history_length):  # 每次加载history_length长度的buffer
  ...
  def train_step(self):
    ...
    for t in range(4):
      ...
      if done:
        break
     observation = frame_buffer.max(0)[0] # (256,256,3)
     self.state_buffer.append(observation) # torch[(256,256,3)]
     if self.training:
        return torch.stack(list(self.state_buffer), 0), reward, done  # stack:沿着一个 新维度 对输入张量序列进行连接,means return (n, 256,256,3)
"""
